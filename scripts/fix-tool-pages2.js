// Regenerate tool pages with CORRECT flat structure: [locale]/tools/{toolId}/page.tsx
const fs = require("fs");
...
// Don't rm - Windows permission issues. Just ensure locale dirs exist.
// if (fs.existsSync(LOCALE_TOOLS_DIR)) {
//   fs.rmSync(LOCALE_TOOLS_DIR, { recursive: true });
// }
fs.mkdirSync(LOCALE_TOOLS_DIR, { recursive: true });

for (const locale of LOCALES) {
  const localeToolDir = path.join(LOCALE_TOOLS_DIR, locale);
  fs.mkdirSync(localeToolDir, { recursive: true });

  for (const [toolId, componentName] of Object.entries(toolMap)) {
    const toolDir = path.join(localeToolDir, toolId);
...
`;
  ...
  console.log(`[${locale}] Generated ${Object.keys(toolMap).length} tool pages at ${localeToolDir}`);
}
