// Generate tool pages: [locale]/tools/{toolId}/page.tsx
const fs = require("fs");
const path = require("path");

const TOOLS_DIR = path.join(__dirname, "..", "src", "app", "tools");
const TOOLS_ROUTE_DIR = path.join(__dirname, "..", "src", "app", "[locale]", "tools");
...
// Ensure tools route dir exists
fs.mkdirSync(TOOLS_ROUTE_DIR, { recursive: true });

// Create ONE page per tool (shared across all locales via [locale] param)
for (const [toolId, componentName] of Object.entries(toolMap)) {
  const toolDir = path.join(TOOLS_ROUTE_DIR, toolId);
...
console.log(`Generated ${Object.keys(toolMap).length} tool pages at ${TOOLS_ROUTE_DIR}`);
