...
import IpLookupClient from "@/app/tools/ip-lookup/IpLookupClient";
...
export default async function ToolPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const tool = tools.find((t) => t.id === TOOL_ID)!;
  return <IpLookupClient locale={locale as Locale} dict={dict} titleOverride={(tool as any).nameTranslations?.[locale] || tool.name} descriptionOverride={(tool as any).descriptionTranslations?.[locale] || tool.description} />;
}
