...
import CoinFlipClient from "@/app/tools/coin-flip/CoinFlipClient";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    keywords: tool.seoKeywords?.join(", ") ?? "", authors: [{ name: SITE_NAME }],
...
export default async function ToolPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <CoinFlipClient locale={locale as Locale} dict={dict} titleOverride={toolTitle} descriptionOverride={toolDescription} />;
}
