...
import ScoreboardClient from "@/app/tools/scoreboard/ScoreboardClient";
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      ...
      languages: { "x-default": `${SITE_URL}/en/tools/${tool.id}`, en: `${SITE_URL}/en/tools/${tool.id}`, es: `${SITE_URL}/es/tools/${tool.id}`, ar: `${SITE_URL}/ar/tools/${tool.id}` },
    },
    openGraph: {
      type: "website", locale: LOCALE_OG[locale] || "en_US", url: `${SITE_URL}/${locale}/tools/${tool.id}`,
      siteName: SITE_NAME, title: `${seoTitle} | ${SITE_NAME}`, description: seoDesc,
...
export default async function ToolPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <ScoreboardClient locale={locale as Locale} dict={dict} titleOverride={toolTitle} descriptionOverride={toolDescription} />;
}
