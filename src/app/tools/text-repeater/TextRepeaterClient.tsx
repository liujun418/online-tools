...
export default function TextRepeaterClient({ locale = "en", dict }: { locale?: string; dict?: Record<string, unknown> } = {}) {
  ...
  const tr = (dict as any)?.textRepeater || {};
    ...
    <ToolLayout {...metadata} locale={locale as any} dict={dict}>
      <div className="flex flex-wrap items-center gap-4">
        <input
          ...
          placeholder={tr.placeholder || "Enter text to repeat..."}
        ...
        >
          <option value="newline">{tr.newLine || "New line"}</option>
          <option value="space">{tr.space || "Space"}</option>
          <option value="comma">{tr.comma || "Comma"}</option>
          <option value="">{tr.noSeparator || "No separator"}</option>
        </select>
      </div>
        ...
        <div className="mt-4">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-sm text-zinc-500 dark:text-zinc-400">
              {(tr.result || "Result ({{count}} characters)").replace("{{count}}", String(result.length))}
            </span>
            <button
