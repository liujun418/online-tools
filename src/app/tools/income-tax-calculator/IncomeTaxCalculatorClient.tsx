...
export default function IncomeTaxCalculatorClient({ locale = "en", dict, titleOverride, descriptionOverride }: {
  ...
  const it = (dict as any)?.incomeTaxCalculator || {};
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label className="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">
            {it.annualGrossIncome || "Annual Gross Income ($)"}
          </label>
          <input
            type="number"
            value={salary}
        ...
        <div>
          ...
          <select
            ...
            className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900"
          >
            <option value="single">{it.single || "Single"}</option>
            <option value="married">{it.married || "Married Filing Jointly"}</option>
          </select>
        </div>
      </div>
      ...
      {result && (
        <>
          <div className="mt-6 rounded-lg bg-blue-600 p-6 text-center text-white">
            <div className="text-sm opacity-80">{it.takeHomePay || "Estimated Annual Take-Home Pay"}</div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-3 text-center dark:border-zinc-800 dark:bg-zinc-900">
              <div className="text-xs text-zinc-500 dark:text-zinc-400">{it.gross || "Gross"}</div>
            ...
            <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-center dark:border-red-800 dark:bg-red-950">
              <div className="text-xs text-red-500 dark:text-red-400">{it.federalTax || "Federal Tax"}</div>
              <div className="text-sm font-bold">${result.federalTax.toLocaleString()}</div>
            </div>
            <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-center dark:border-red-800 dark:bg-red-950">
              <div className="text-xs text-red-500 dark:text-red-400">{it.fica || "FICA (SS + Medicare)"}</div>
            ...
            <div className="rounded-lg border border-green-200 bg-green-50 p-3 text-center dark:border-green-800 dark:bg-green-950">
              <div className="text-xs text-green-500 dark:text-green-400">{it.effectiveRate || "Effective Rate"}</div>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 text-center dark:border-zinc-800 dark:bg-zinc-900">
              <div className="text-sm text-zinc-500 dark:text-zinc-400">{it.monthlyNet || "Monthly Net Pay"}</div>
              <div className="text-xl font-bold text-zinc-900 dark:text-white">${result.monthlyNet.toLocaleString()}</div>
            </div>
            <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 text-center dark:border-zinc-800 dark:bg-zinc-900">
              <div className="text-sm text-zinc-500 dark:text-zinc-400">{it.biweeklyNet || "Biweekly Net Pay"}</div>
