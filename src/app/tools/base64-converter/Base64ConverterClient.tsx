...
export default function Base64ConverterClient({ locale = "en", dict }: { locale?: string; dict?: Record<string, unknown> } = {}) {
  ...
  const b64 = (dict as any)?.base64Converter || {};
  ...
  return (
    <ToolLayout {...metadata} locale={locale as any} dict={dict}>
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => { setMode("decode"); setOutput(""); setError(null); }}
          className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${mode === "decode" ? "bg-blue-600 text-white" : "bg-zinc-200 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300"}`}>
          {b64.decode || "Decode"}
        </button>
        <button onClick={handleSwap} className="ml-2 rounded-lg bg-zinc-200 px-4 py-2 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-300 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700">
          {b64.swap || "Swap"}
        </button>
      </div>
      ...
      <div className="mt-4">
        ...
        <textarea
          ...
          placeholder={mode === "encode" ? (b64.encodePlaceholder || "Enter text to encode...") : (b64.decodePlaceholder || "Enter Base64 to decode...")}
      ...
      <button onClick={handleConvert} disabled={!input.trim()} className="mt-4 rounded-lg bg-blue-600 px-6 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700 disabled:opacity-50">
        {mode === "encode" ? (b64.encode || "Encode") : (b64.decode || "Decode")}
      </button>
