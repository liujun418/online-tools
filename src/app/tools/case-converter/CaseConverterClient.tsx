...
export default function CaseConverterClient({ locale = "en", dict, titleOverride, descriptionOverride }: {
  ...
  const cases: { label: string; fn: (s: string) => string }[] = [
    { label: cc.uppercase || "UPPERCASE", fn: (s) => s.toUpperCase() },
    { label: cc.lowercase || "lowercase", fn: (s) => s.toLowerCase() },
    { label: cc.titleCase || "Title Case", fn: (s) => s.replace(/\w\S*/g, (t) => t.charAt(0).toUpperCase() + t.slice(1).toLowerCase()) },
    { label: cc.sentenceCase || "Sentence case", fn: (s) => s.toLowerCase().replace(/(^\s*\w|[.!?]\s*\w)/g, (c) => c.toUpperCase()) },
    { label: cc.alternating || "aLtErNaTiNg", fn: (s) => s.split("").map((c, i) => (i % 2 === 0 ? c.toLowerCase() : c.toUpperCase())).join("") },
    { label: cc.invert || "iNVERT", fn: (s) => s.split("").map((c) => (c === c.toUpperCase() ? c.toLowerCase() : c.toUpperCase())).join("") },
  ];
        ...
        {cases.map((c) => (
          ...
          >
            ...
            <div className="text-sm text-zinc-700 dark:text-zinc-300">
              {text || <span className="text-zinc-400 italic">{cc.emptyText || "Your text will appear here"}</span>}
            </div>
          </div>
        ))}
      </div>
